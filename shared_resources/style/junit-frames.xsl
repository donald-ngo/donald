...
<xsl:template match="testsuites">
  ...
  <!-- create the alltests-skipped.html at the root -->
    <redirect:write file="{$output.dir}/alltests-skipped.html">
      <xsl:apply-templates select="." mode="all.tests">
        <!-- DN: This queries for all tests marked at 'skipped' -->
...
<xsl:template match="testsuites" mode="all.tests">
	...
	<head>
	    <title>Automated Test Results: <xsl:value-of select="$title"/></title>
...
<xsl:template match="testsuite" mode="class.details">
    ...
    <html>
        <head>
          <title>Selenium Test Results: <xsl:value-of select="$class.name"/></title>
            <xsl:call-template name="create.stylesheet.link">
...
<xsl:template match="testsuites" mode="overview.packages">
    <html>
        <head>
            <title>Automated Test Results: Summary</title>
            <xsl:call-template name="create.stylesheet.link">
                <xsl:with-param name="package.name"/>
            </xsl:call-template>
        </head>
        ...
        <xsl:variable name="skippedCount" select="sum(testsuite/@skip)"/>
        <xsl:variable name="errorCount" select="sum(testsuite/@errors)"/>
        <xsl:variable name="failureCount" select="sum(testsuite/@failures)"/>
        <xsl:variable name="timeCount" select="sum(testsuite/@time)"/>
        <xsl:variable name="successRate" select="($testCount - $skippedCount - $failureCount - $errorCount) div $testCount"/>
        <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
        <tr valign="top">
            <th>Tests</th>
            <th>Skips</th>
        ...
        <tr valign="top">
            ...
            <td><a title="Display all skipped" href="alltests-skipped.html"><xsl:value-of select="$skippedCount"/></a></td>
        ...
        <table class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
            ...
            <xsl:for-each select="testsuite[not(./@package = preceding-sibling::testsuite/@package)]">
                ...
                <tr valign="top">
                    ...
                    <td><xsl:value-of select="sum($insamepackage/@skip)"/></td>
...
<xsl:template name="pageHeader">
    ...
    <tr>
        ...
        <td align="right">Please contact the <a href="mailto:csbu.toqa@@hp.com">QA Team</a> for any questions.</td>
...
<xsl:template name="testsuite.test.header">
    <tr valign="top">
        ...
        <th>Errors</th>
        <th>Failures</th>
        <th nowrap="nowrap">Time (minutes)</th>
...
<xsl:template match="testsuite" mode="print.test">
    <tr valign="top">
		...
		<xsl:when test="@skip != 0">
		    <a title="Display only skipped" href="{@id}_{@name}-skipped.html"><xsl:apply-templates select="@skip"/></a>
		</xsl:when>
		<xsl:otherwise>
		<!-- DN: This shoes up in the Classes section for Skips -->
		    0<!--<xsl:apply-templates select="@skip"/>-->
		</xsl:otherwise>
	    </xsl:choose>
	</td>
        <td>
	...
	<xsl:if test="boolean($show.class)">
        ...
        <xsl:choose>
            <xsl:when test="skipped">
                <td>Skipped</td>
                <!-- DN: This is where the Skip description displays -->	
        ...
        <!-- DN:ADDED -->
        <td>
            <xsl:variable name="donald">
	    		<xsl:value-of select="concat(translate(../@package,'.','.'),'.',../@name,'$1','_',@name)"/>
			</xsl:variable>
			
			<xsl:variable name="donald2">
	    		<xsl:value-of select="concat('',../@name,'$1','_',@name)"/>
			</xsl:variable>
			...
			<xsl:choose>
        		<xsl:when test="contains(translate(concat(translate(../@package,'.','.'),'.',../@name,'$1','_',@name),$smallcase, $uppercase),'WEB') or contains(translate(concat(translate(../@package,'.','.'),'.',../@name,'$1','_',@name),$smallcase, $uppercase),'E2E')">
        			<xsl:choose>
            			<xsl:when test="failure">
	        				<a href="{concat(translate(../@package,'.','.'),'.',../@name,'$1','_',@name)}.png">Screenshot</a>
	        			</xsl:when> 
            			<xsl:when test="error">
	        				<a href="{concat(translate(../@package,'.','.'),'.',../@name,'$1','_',@name)}.png">Screenshot</a>
	        			</xsl:when>
...
<xsl:template match="skipped">
    <xsl:call-template name="display-skipped"/>
</xsl:template>
...
<xsl:template name="display-skipped">
    <xsl:choose>
        <xsl:when test="not(@message)">N/A</xsl:when>
        <xsl:otherwise>
            <xsl:value-of select="@message"/>
...
<xsl:template name="display-time">
    ...
    <!--DN: 2/3 <xsl:value-of select="format-number($value,'0.000')"/> -->
    <xsl:value-of select="format-number($value div 60,'0.000')"/>
</xsl:template>
